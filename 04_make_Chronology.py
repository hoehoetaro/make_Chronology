...
def csv_to_html_table(csv_path):
    table_html = '<h5>{}</h5>\n'.format(escape(os.path.splitext(os.path.basename(csv_path))[0]))
    table_html += '<table>\n'
    with open(csv_path, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        headers = next(reader)  # get the first line
        table_html += '<thead>\n<tr>\n'  # start the thead section
        for header in headers:  # for each cell in the first row
            table_html += f'<th>{escape(header)}</th>\n'  # create a th tag
        table_html += '</tr>\n</thead>\n'  # end the thead section
        for row in reader:  # for each subsequent row
            table_html += '<tr>\n'
            for cell in row:
                table_html += f'<td>{escape(cell)}</td>\n'
            table_html += '</tr>\n'
    table_html += '</table>\n'
    return table_html
...
with open('HTML_data.csv', 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if command == 'title':
            ...
            procedure_content += f'<div class="div-table">{csv_to_html_table(content)}</div>\n'
